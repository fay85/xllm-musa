...
MusaDenseMLPImpl::MusaDenseMLPImpl(int64_t hidden_size,
  ...
  musa::LinearExtraArgs gate_up_proj_extra_args("none", false);
  musa::LinearExtraArgs down_proj_extra_args("none", false);
  if (is_smoothquant_) {
    // For per-token smoothquant, use specific args
    down_proj_extra_args = musa::LinearExtraArgs(hidden_act_, is_gated_);
  }
  ...
  gate_up_proj_ =
      register_module("gate_up_proj",
                      musa::ColumnParallelLinear(hidden_size,
                                                 out_feature,
                                                 /*bias=*/has_bias,
                                                 /*gather_output=*/false,
                                                 quant_args,
  ...
  act_ = register_module(
      "act", musa::Activation(hidden_act_, is_gated_, swiglu_limit_));
  ...
  down_proj_ =
      register_module("down_proj",
                      musa::RowParallelLinear(intermediate_size_,
                                              hidden_size,
                                              /*bias=*/has_bias,
...
torch::Tensor MusaDenseMLPImpl::forward(const torch::Tensor& hidden_states) {
...
void MusaDenseMLPImpl::load_state_dict(const StateDict& state_dict) {
...
void MusaDenseMLPImpl::load_state_dict(
...
std::optional<torch::Tensor> MusaDenseMLPImpl::get_fp8_input_scale() const {
  if (gate_up_proj_) {
