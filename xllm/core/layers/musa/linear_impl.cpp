...
// Detect DeepSeek-style block-wise FP8 quantization.  The backend
// implementation lives in layers/musa/linear.cpp; keeping the format check
// here lets the shared Linear loading code select the backend path.
...
torch::Tensor block_fp8_forward_dispatch(
    const torch::Tensor& input,
    ...
    MatmulOutputBuffers& output_buffers) {
#if defined(USE_MUSA)
  return ::xllm::layer::musa::block_fp8_forward(input,
                                                weight_fp8,
                                                weight_scale_inv,
                                                weight_block_size,
                                                bias,
                                                output_buffers);
#else
  LOG(FATAL) << "block-wise FP8 linear is only implemented for MUSA";
  return torch::Tensor();
#endif
}
...
torch::Tensor ColumnParallelLinearImpl::forward(torch::Tensor input) {
  ...
  if (quant_args_.quant_method() == kQuantMethodSmoothquant) {
    ...
    if (weight_.scalar_type() == torch::kFloat8_e4m3fn) {
      output = block_fp8_forward_dispatch(input,
                                          weight_,
                                          weight_scale_inv_,
      ...
      output = xllm::kernel::musa::matmul(matmul_params);
    }
  } else if (quant_args_.quant_method() == kQuantMethodFp8) {
...
torch::Tensor ReplicatedLinearImpl::forward(torch::Tensor input) {
  ...
  auto output = xllm::kernel::musa::matmul(matmul_params);
  return output;
}
