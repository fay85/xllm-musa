...
std::vector<int64_t> expand_int_vector_to_batch(const std::vector<int64_t>& src,
  ...
  CHECK_EQ(target_batch % source, 0)
      << name << " cannot be expanded from " << source << " to " << target_batch;
...
torch::Tensor run_spec_verify_conv(const torch::Tensor& mixed_qkv,
  ...
  auto state_indices =
      expand_sequence_tensor_to_batch(
          ...
          .contiguous();
  const std::vector<int64_t> accepted_host =
      expand_int_vector_to_batch(num_accepted_host, batch_size,
                                 "num_accepted_tokens");
  ...
  const bool defer_conv_commit = verify_cache != nullptr && verify_cache->enabled;
  if (defer_conv_commit) {
    conv_intermediate =
        torch::empty({batch_size, seq_len, dim, expanded_state_len},
                     conv_cache.options());
  }
  ...
  for (int64_t seq_idx = 0; seq_idx < batch_size; ++seq_idx) {
    ...
    auto full_state =
        conv_cache.index_select(/*dim=*/0, cache_idx)
            .select(/*dim=*/0, 0)
            .to(torch::kFloat32)
            .contiguous();
    auto history =
    ...
    for (int64_t token_idx = 0; token_idx < seq_len; ++token_idx) {
      ...
      if (defer_conv_commit) {
        ...
        if (old_prefix_len > 0) {
          step_state
              .narrow(/*dim=*/-1, /*start=*/0, /*length=*/old_prefix_len)
              .copy_(full_state
                         .narrow(/*dim=*/-1,
        ...
        const int64_t committed_len = token_idx + 1;
        step_state
            .narrow(/*dim=*/-1,
                    /*start=*/old_prefix_len,
                    /*length=*/committed_len)
            .copy_(mixed_qkv.select(0, seq_idx)
                       .narrow(/*dim=*/-1, /*start=*/0, /*length=*/committed_len)
                       .to(conv_cache.scalar_type()));
        conv_intermediate.select(0, seq_idx)
            .select(0, token_idx)
            .copy_(step_state);
      }
    }
    ...
    if (!defer_conv_commit) {
      ...
      next_state
          .narrow(/*dim=*/-1, /*start=*/old_prefix_len, /*length=*/seq_len)
...
torch::Tensor run_spec_verify_gated_delta_rule(
  ...
  for (int64_t seq_idx = 0; seq_idx < batch_size; ++seq_idx) {
    ...
    auto init_state_index =
        checkpoint_indices_long.select(0, seq_idx)
    ...
    auto recurrent_state =
        ssm_cache.index_select(/*dim=*/0, init_state_index)
            .to(torch::kFloat32)
            .contiguous();
    if (transpose_state) {
    ...
    for (int64_t token_idx = 0; token_idx < seq_len; ++token_idx) {
      auto q_t =
          query.select(0, seq_idx).select(0, token_idx).unsqueeze(0);
      ...
      auto kv_mem =
          (recurrent_state * k_t.unsqueeze(-1)).sum(/*dim=*/-2);
      auto delta = (v_t - kv_mem) * beta_t;
      recurrent_state =
          recurrent_state + k_t.unsqueeze(-1) * delta.unsqueeze(-2);
      auto token_out =
          (recurrent_state * q_t.unsqueeze(-1)).sum(/*dim=*/-2);
...
bool gdn_packed_layout_debug_enabled() {
  static const bool enabled = [] {
    const char* env = std::getenv("XLLM_GDN_PACKED_DEBUG");
    return env != nullptr && env[0] == '1';
  }();
  return enabled;
}

bool use_custom_prefill_conv_kernel() {
  static const bool enabled = [] {
...
double gdn_padded_waste_ratio(const std::vector<int32_t>& cu,
                              int64_t max_len) {
...
bool cu_seqlens_all_full(c10::ArrayRef<int32_t> cu, int64_t max_len) {
  ...
  for (int64_t i = 0; i < num_seqs; ++i) {
    const int64_t len = static_cast<int64_t>(cu[static_cast<size_t>(i) + 1] -
                                             cu[static_cast<size_t>(i)]);
    if (len != max_len) {
      return false;
    }
  }
...
torch::Tensor pad_packed_4d_to_rect(const torch::Tensor& packed,
...
torch::Tensor pad_packed_3d_to_rect(const torch::Tensor& packed,
                                    c10::ArrayRef<int32_t> cu,
                                    int64_t max_len) {
  CHECK_EQ(packed.dim(), 3);
  CHECK_EQ(packed.size(0), 1);
  const int64_t num_seqs = static_cast<int64_t>(cu.size()) - 1;
  if (cu_seqlens_all_full(cu, max_len)) {
    CHECK_EQ(packed.size(1), num_seqs * max_len);
    return packed.reshape({num_seqs, max_len, packed.size(2)});
  }
  auto out =
      torch::zeros({num_seqs, max_len, packed.size(2)}, packed.options());
  for (int64_t seq_idx = 0; seq_idx < num_seqs; ++seq_idx) {
    const int64_t start =
        static_cast<int64_t>(cu[static_cast<size_t>(seq_idx)]);
    const int64_t end =
...
torch::Tensor unpad_rect_4d_to_packed(const torch::Tensor& padded,
                                      c10::ArrayRef<int32_t> cu) {
...
bool should_use_gdn_packed_prefill(
...
bool use_mate_gdn_prefill_kernel() {
  static const bool enabled = [] {
    const char* v = std::getenv("XLLM_MATE_GDN_PREFILL");
    return (v == nullptr) || (v[0] != '0');
  }();
  return enabled;
}
...
void scatter_gdn_mtp_verify_ssm_states(
  ...
  torch::Tensor accepted_cpu =
      accepted_tokens.to(torch::kCPU, torch::kLong).contiguous();
  CHECK_EQ(accepted_cpu.dim(), 2) << "accepted tokens must be [batch, width]";
  const int64_t num_sequences = accepted_cpu.size(0);
  const int64_t token_width = accepted_cpu.size(1);
  const int64_t* accepted_data = accepted_cpu.const_data_ptr<int64_t>();

  std::vector<int32_t> accepted_steps(static_cast<size_t>(num_sequences));
  for (int64_t seq_idx = 0; seq_idx < num_sequences; ++seq_idx) {
    int32_t accepted_len = 0;
    const int64_t row_offset = seq_idx * token_width;
    for (int64_t token_idx = 0; token_idx < token_width; ++token_idx) {
      if (accepted_data[row_offset + token_idx] < 0) {
        break;
      }
      ++accepted_len;
    }
    CHECK_GT(accepted_len, 0)
        << "each sequence must accept at least one target token";
  ...
  if (!linear_state_indices.defined()) {
    CHECK(!input_params.embedding.linear_state_ids.empty())
        << "linear_state_ids required for GDN MTP post-verify scatter";
    linear_state_indices =
        torch::tensor(input_params.embedding.linear_state_ids, torch::kInt32)
            .to(device);
  }
  ...
  for (const auto& [layer_id, state] : cache.layer_states) {
    ...
    torch::Tensor logical_indices = expand_sequence_tensor_to_batch(
        linear_state_indices, batch_size, "linear_state_indices");
    torch::Tensor base_indices =
        build_linear_state_base_indices(logical_indices, checkpoint_stride);
    ...
    for (int64_t seq_idx = 0; seq_idx < batch_size; ++seq_idx) {
      const int32_t step =
          accepted_steps[static_cast<size_t>(seq_idx)];
      CHECK_GE(step, 0);
      CHECK_LT(step, seq_len)
          << "accepted step must fit intermediate verify length";
      if (ssm_cache.defined() && ssm_cache.numel() > 0) {
        const int64_t dst_slot =
            base_indices[seq_idx].item<int64_t>() + static_cast<int64_t>(step);
        torch::Tensor state_vk =
            intermediate.select(0, seq_idx).select(0, step);
        torch::Tensor state_to_store =
            fla_ssm_state_layout ? state_vk.transpose(-1, -2).contiguous()
                                 : state_vk;
        ssm_cache.index_copy_(
            /*dim=*/0,
            torch::tensor({dst_slot}, torch::kLong).to(device),
            state_to_store.unsqueeze(0).to(ssm_cache.scalar_type()));
      }
      if (scatter_conv) {
        const int64_t conv_dst =
            logical_indices[seq_idx].item<int64_t>();
        torch::Tensor conv_state =
            conv_intermediate.select(0, seq_idx).select(0, step);
        conv_cache.index_copy_(
            /*dim=*/0,
            torch::tensor({conv_dst}, torch::kLong).to(device),
            conv_state.unsqueeze(0).to(conv_cache.scalar_type()));
      }
    }
  }
}
...
torch::Tensor Qwen3GatedDeltaNetBaseImpl::forward_packed_prefill(
  ...
  // Prefill token counts exceed the decode graph buffer cap; leave buffers
  // undefined so the kernel allocates (eager-safe).
...
torch::Tensor Qwen3GatedDeltaNetBaseImpl::forward(
  ...
  if (!use_custom_prefill_conv && !use_spec_verify &&
    ...
    if (cache_state_len == history_len) {
      ...
      conv_state = torch::zeros(
          {batch_size, conv_in.size(1), cache_state_len}, conv_in.options());
    ...
    // capturing".
    {
      ...
      if (needs) {
        const int64_t target_M = conv1d_decode_out_buf_.defined()
                                     ? std::max(M, conv1d_decode_out_buf_.size(0))
                                     : M;
        conv1d_decode_out_buf_ =
            torch::empty({target_M, D}, x_in.options());
      }
  ...
  if (use_spec_verify || checkpoint_stride > 1) {
    ...
    if (qwen35_mtp_debug_enabled() && use_spec_verify) {
      LOG(INFO) << "[Qwen3.5 MTP debug] forward after gating: a="
                << a.sizes() << ", b=" << b.sizes() << ", g=" << g.sizes()
                << ", beta=" << beta.sizes();
    }
  ...
  if (use_mate_gdn_prefill) {
    ...
    if (!attn_metadata.q_cu_seq_lens_host_vec.empty()) {
      mate_params.cu_seqlens_host = attn_metadata.q_cu_seq_lens_host_vec;
    ...
    if (qwen35_mtp_debug_enabled()) {
      LOG(INFO) << "[Qwen3.5 MTP debug] forward spec gdn: processed_q="
                << processed_q.sizes() << ", processed_k="
                << processed_k.sizes() << ", processed_v="
                << processed_v.sizes() << ", ssm_cache="
                << ssm_cache.sizes()                 << ", checkpoint_indices="
                << checkpoint_indices.sizes()
                << ", num_accepted_host.size()="
                << input_params.num_accepted_tokens_host.size()
                << ", q_cu_seq_lens=" << attn_metadata.q_cu_seq_lens.sizes();
    }
    qwen35_mtp_debug_sync("forward_before_spec_gdn");
    // The fused mate MTP kernel is JIT-specialized on the validate token count
    // (deployed for seq_len == 2, i.e. one draft token). Fall back to the host
    // loop for any other length until the corresponding kernel is deployed.
    if (use_mate_gdn_mtp_kernel() && seq_len == 2) {
      const bool mtp_cmp = qwen35_mtp_debug_enabled();
      torch::Tensor ssm_clone;
      if (mtp_cmp) {
      ...
      if (mtp_cmp) {
        auto out_host = run_spec_verify_gated_delta_rule(
            ...
            checkpoint_indices, input_params.num_accepted_tokens_host,
            attn_metadata.q_cu_seq_lens, attn_metadata.q_seq_lens_vec,
            fla_ssm_state_layout, scale);
        auto out_diff = (core_attn_out.to(torch::kFloat32) -
                         out_host.to(torch::kFloat32))
                            .abs()
                            .max()
                            .item<double>();
        auto sd = (ssm_cache.to(torch::kFloat32) -
                   ssm_clone.to(torch::kFloat32))
                      .abs();
        LOG(INFO) << "[MTP fused-vs-host] out_max|diff|=" << out_diff
                  << " state_max|diff|=" << sd.max().item<double>()
                  << " state_mean|diff|=" << sd.mean().item<double>()
                  << " ssm_fused_absmax=" << ssm_cache.abs().max().item<double>()
                  << " ssm_host_absmax=" << ssm_clone.abs().max().item<double>()
    ...
    // Shape: [batch_size, num_heads, head_k_dim, head_v_dim] (or V/K swapped).
    torch::Tensor initial_state_tensor =
        torch::index_select(ssm_cache, 0, linear_state_base_indices);
    CHECK_EQ(input_params.parallel.has_initial_state.size(),
    ...
    // MUSA chunk_gated_delta_rule does not honor cu_seqlens yet (unlike NPU
    // mega_chunk_gdn). Run one sequence at a time so initial_state batch matches
    // the packed [1, T, H, D] tensors in MIXED/CHUNKED_PREFILL batches.
    core_attn_out = torch::zeros_like(processed_v);
    std::vector<torch::Tensor> per_seq_states;
    per_seq_states.reserve(static_cast<size_t>(batch_size));
    for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
      ...
      chunk_gated_delta_params.initial_state =
          initial_state_tensor.select(0, batch_idx).unsqueeze(0);
      chunk_gated_delta_params.output_final_state = true;
      chunk_gated_delta_params.head_first = false;
      chunk_gated_delta_params.use_qk_l2norm_in_kernel = true;
      torch::Tensor seq_core_attn_out;
      torch::Tensor seq_last_state;
      std::tie(seq_core_attn_out, seq_last_state) =
          xllm::kernel::chunk_gated_delta_rule(chunk_gated_delta_params);
      core_attn_out[batch_idx]
          .narrow(/*dim=*/0, /*start=*/0, valid_len)
          .copy_(seq_core_attn_out[0].narrow(/*dim=*/0, /*start=*/0,
                                              valid_len));
