...
void populate_attention_metadata(
  ...
  if (attn_metadata.use_expanded_decode_for_spec_verify_attention) {
    ...
    // Expanded MTP verification has its own logical paged-KV layout.  Route
    // its CPU mirrors through the generic decode fields consumed by the Mate
    // FFI wrapper; leaving the ordinary request mirrors here would make the
    // wrapper fall back to a device-to-host copy (and pass the wrong block
    // table) during graph capture/replay.
    if (params.graph.expanded_paged_kv_indptr_host.defined()) {
      attn_metadata.musa.paged_kv_indptr_host =
          params.graph.expanded_paged_kv_indptr_host;
    }
    if (params.graph.expanded_paged_kv_indices_host.defined()) {
      attn_metadata.musa.paged_kv_indices_host =
          params.graph.expanded_paged_kv_indices_host;
    }
