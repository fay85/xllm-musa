...
void update_llm_decode_metadata_from_host(
  ...
  if (padded_num_tokens > actual_num_tokens) {
    const size_t pad_bytes = static_cast<size_t>(padded_num_tokens -
                                                 actual_num_tokens) *
                             sizeof(int32_t);
    const cudaError_t zero_tokens =
        cudaMemsetAsync(params.dst_tokens + actual_num_tokens,
                        0,
                        pad_bytes,
                        stream);
    ...
    const cudaError_t zero_slots =
        cudaMemsetAsync(params.dst_new_cache_slots + actual_num_tokens,
                        0,
                        pad_bytes,
                        stream);
