...
void causal_conv1d_fwd_kernel(ConvParamsBase params) {
    ...
    input_t *conv_states = params.conv_state_ptr == nullptr ? nullptr
        : reinterpret_cast<input_t *>(params.conv_state_ptr) + cache_index * params.conv_state_batch_stride + channel_id * params.conv_state_c_stride;
...
__global__ void causal_conv1d_fwd_token_major_kernel(ConvParamsBase params) {
    ...
    auto load_x = [&](int token_idx) {
        return static_cast<float>(
            x[(seq_start + token_idx) * params.x_l_stride +
        ...
        if (seq_len >= 3) {
            ...
            conv_state[0 * params.conv_state_l_stride] =
                load_initial_state
                    ...
                    : input_t(0.0f);
            conv_state[1 * params.conv_state_l_stride] =
                x[seq_start * params.x_l_stride +
                  feature * params.x_c_stride];
            conv_state[2 * params.conv_state_l_stride] =
                x[(seq_start + 1) * params.x_l_stride +
                  feature * params.x_c_stride];
        } else {
            const input_t previous1 =
                conv_state[1 * params.conv_state_l_stride];
            const input_t previous2 =
                conv_state[2 * params.conv_state_l_stride];
            conv_state[0 * params.conv_state_l_stride] =
...
void causal_conv1d_fwd(
    ...
    set_conv_params_fwd(params, batch_size, dim, seqlen, width, x, weight, out,
                        bias_, silu_activation, pad_slot_id,
