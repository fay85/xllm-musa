...
def host_cumsum(g_log, cs=64):
    ...
    B,T,H = x.shape
    pad = (cs - T % cs) % cs
    if pad: x = F.pad(x, (0,0,0,pad), value=0.0)
    Tp = x.shape[1]
...
torch.manual_seed(0)
device="musa"
T,Hq,Hv,D=11,16,48,128
base=torch.randn(1,T,1,D,device=device,dtype=torch.bfloat16)
q=(base + 0.01*torch.randn(1,T,Hq,D,device=device,dtype=torch.bfloat16)).contiguous()
k=(base + 0.01*torch.randn(1,T,Hq,D,device=device,dtype=torch.bfloat16)).contiguous()
v=torch.randn(1,T,Hv,D,device=device,dtype=torch.bfloat16)
g=-torch.rand(1,T,Hv,device=device,dtype=torch.float32)*8
beta=torch.sigmoid(torch.randn(1,T,Hv,device=device,dtype=torch.float32))
o_m,ht_m=run_ffi(q,k,v,g,beta)
o_r,ht_r=recurrent(q,k,v,g,beta)
print("simple-ffi vs recurrent corr-keys", float((o_m.float()-o_r).abs().max()), float((o_m.float()-o_r).abs().mean()))
print("nan mate", bool(torch.isnan(o_m.float()).any()), "max", float(o_m.float().abs().max()))
