...
# The MUSA performance path uses the stable prefill bucket graph by default.
# Each switch remains explicitly overridable for correctness bisection.
ENABLE_PREFILL_PIECEWISE_GRAPH="${ENABLE_PREFILL_PIECEWISE_GRAPH:-1}"
ENABLE_PACKED_PREFILL="${ENABLE_PACKED_PREFILL:-1}"
# Keep packed prefill groups small and stable.  This preserves the larger
# decode batch capacity while avoiding a new B-shaped GDN graph for every
# arrival partition (C=5 becomes the reusable 2+2+1 pattern).
export XLLM_MAX_PACKED_PREFILL_SEQS="${XLLM_MAX_PACKED_PREFILL_SEQS:-2}"
export XLLM_PACKED_PREFILL_PIECEWISE="${XLLM_PACKED_PREFILL_PIECEWISE:-1}"
export XLLM_USE_FA3="${XLLM_USE_FA3:-1}"
export XLLM_USE_FA3_DECODE="${XLLM_USE_FA3_DECODE:-1}"
...
setup_runtime_env() {
  ...
  export LD_LIBRARY_PATH="${MUDNN_HOME}/lib:/usr/local/lib/python3.10/dist-packages/tvm_ffi/lib:/usr/local/lib/python3.10/dist-packages/torch_musa/lib:/usr/local/lib/python3.10/dist-packages/torch/lib:/usr/local/musa/lib:${MCCL_HOME}/lib:/usr/local/openmpi/lib:${LD_LIBRARY_PATH:-}"
  ...
  export LD_LIBRARY_PATH="${MUDNN_HOME}/lib:${MATE_FFI_ROOT}/mate_flashinfer_prefill_ffi:${MATE_FFI_ROOT}/mate_flashinfer_batch_attention_ffi:${MATE_FFI_ROOT}/mate_flashinfer_decode_ffi:/opt/intel/oneapi/mkl/lib/intel64:/usr/local/lib/python3.10/dist-packages/tvm_ffi/lib:/usr/local/lib/python3.10/dist-packages/torch_musa/lib:/usr/local/lib/python3.10/dist-packages/torch/lib:/usr/local/musa/lib:${MCCL_HOME}/lib:/usr/local/openmpi/lib:${LD_LIBRARY_PATH:-}"
...
check_flashinfer_ops() {
  ...
  case "$MODEL_NAME" in
    Qwen3.5-35B-A3B|Qwen3.5-35B-A3B-FP8|qwen3.5-35b-a3b|qwen3.5-35b-a3b-fp8)
      # Mate tunes GDN decode for continuous batch ranges. Reusing the B<=2
      # TileLang module for B>=3 changes the grid-to-state mapping and corrupts
      # recurrent state, so all runtime buckets are required.
      uris+=(
        "mate_gdn_decode_tvmffi_hq16_hv32_bf16"
        "mate_gdn_decode_tvmffi_hq16_hv32_bf16_b4"
        "mate_gdn_decode_tvmffi_hq16_hv32_bf16_b16"
        "mate_gdn_decode_tvmffi_hq16_hv32_bf16_blarge"
      )
      ;;
  esac
  ...
  if [[ "$missing" -eq 1 ]]; then
    ...
    if [[ "$MODEL_NAME" == Qwen3.5-35B-A3B* ||
          "$MODEL_NAME" == qwen3.5-35b-a3b* ]]; then
      echo "         Export the batch-tuned GDN decode modules with:" >&2
      echo "           python tools/export_mate_gdn_decode.py --ops-path ${ops_root}" >&2
    fi
  fi
}
...
model_runtime_flags() {
  ...
  case "${model_name,,}" in
    qwen3.5-35b-a3b*|qwen3.6-35b-a3b*)
      # BF16 35B-A3B weights leave only a small KV-cache budget at 0.70.
  ...
  case "$model_name" in
    Qwen3.5-27B|Qwen3.5-27b|qwen3.5-27b|Qwen3.5-27B-FP8|Qwen3.5-27B-fp8|qwen3.5-27b-fp8|Qwen3.5-35B-A3B|Qwen3.5-35B-A3B-FP8|qwen3.5-35b-a3b|qwen3.5-35b-a3b-fp8|Qwen3.6-35B-A3B|Qwen3.6-35B-A3B-FP8|qwen3.6-35b-a3b|qwen3.6-35b-a3b-fp8)
...
run_xllm() {
  ...
  local -a cmd=(
    ...
    "--enable_adaptive_prefill_oneshot=${ENABLE_ADAPTIVE_PREFILL_ONESHOT:-false}"
  ...
  if [[ "${ENABLE_GRAPH:-1}" == "1" ]]; then
    ...
    echo "==> Graph mode: enable_graph=true vmm_pool=${ENABLE_GRAPH_VMM_POOL:-0} prefill_piecewise=${ENABLE_PREFILL_PIECEWISE_GRAPH:-0} packed_prefill=${ENABLE_PACKED_PREFILL:-0} adaptive_oneshot=${ENABLE_ADAPTIVE_PREFILL_ONESHOT:-false}"
  else
    # The C++ flag defaults to true, so pass false explicitly when the
    # environment requests eager execution (also required for eager MTP).
    cmd+=("--enable_graph=false")
    echo "==> Graph mode: enable_graph=false"
  fi
  ...
  # Then launch with separate draft checkpoint (MTP graph verify is enabled by default):
  ...
  #     ENABLE_GRAPH=1 XLLM_ENABLE_GRAPH_MTP=1 bash run_xllm_musa.sh ...
  ...
  local enable_graph_mtp="${XLLM_ENABLE_GRAPH_MTP:-1}"
  if [[ "${ENABLE_GRAPH:-1}" == "1" && "$num_spec" -gt 0 &&
        "$enable_graph_mtp" != "1" ]]; then
    echo "==> WARNING: speculative decoding (MTP) is disabled by XLLM_ENABLE_GRAPH_MTP=0"
    num_spec=0
  fi
  if [[ "$num_spec" -gt 0 ]]; then
    ...
    if [[ "${ENABLE_GRAPH:-1}" == "1" ]]; then
      echo "==> Graph MTP:              enabled (independent spec-verify graph)"
    fi
  fi
